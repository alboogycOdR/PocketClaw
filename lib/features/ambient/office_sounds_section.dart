/// Office Sounds section — 9-channel mixer streamed from
/// soundofcolleagues.com. Two-column layout with per-channel sliders
/// and +/− step buttons, matching the original web aesthetic.
library;
...
class OfficeSoundsSection extends ConsumerWidget {
  const OfficeSoundsSection({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final stateAsync = ref.watch(officeSoundStateProvider);
    final state = stateAsync.valueOrNull;
    final isPlaying = state?.isPlaying ?? false;
    final volumes = state?.volumes ?? List.filled(officeSoundChannels.length, 0.5);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // Left: 4 channels, Right: 5 channels
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // Left column
              ...
              Expanded(
                child: Column(
                  ...
                  children: List.generate(5, (i) {
                    final idx = i + 4;
                    return _ChannelSlider(
                      channel: officeSoundChannels[idx],
                      index: idx,
                      volume: volumes[idx],
                    );
                  }),
...
class _PlayStopButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => officeSoundEngine.toggle(),
...
class _ChannelSlider extends StatelessWidget {
  final OfficeSoundChannel channel;
  final int index;
  final double volume;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              _StepButton(
                label: '−',
                onTap: () => officeSoundEngine.nudgeVolume(index, -0.1),
              ),
              const SizedBox(width: 4),
              // Slider
              Expanded(
                child: SliderTheme(
                  ...
                  child: Slider(
                    ...
                    max: 1.0,
                    onChanged: (v) =>
                        officeSoundEngine.setChannelVolume(index, v),
                  ),
                ),
              ),
              const SizedBox(width: 4),
...
// ── Square thumb shape (matches original web slider aesthetic) ────────────────
